...
def topology(atnames, bonds, ffbond):
    ...
    Returns:
        ...
        excl (array): exclusion list of bonded atom indices for each atom
    """
...
def ekinetic(m, v):
    ...
    ekin = 1/2 * np.sum(m * v * v) * 1e4 # [kJ/mol]
    temp = ekin / (3/2 * len(m) * cst.R * 1e-3)
    return ekin, temp
...
def forces_pair(r, sig, eps, box, rcut, excl):
    ...
    for i in range(natom - 1):
        for j in range(i + 1, natom):
            ...
            sr6 = sr2 * sr2 * sr2
            sr12 = sr6 * sr6
            fij = 4 * epsij * (12 * sr12 - 6 * sr6) / rsq * rij
            f[i] += fij
            f[j] -= fij
            vir += np.sum(rij * fij)
    return f, vir
...
def forces_bond(r, bonds, box):
    ...
    for bd in bonds:
        rij = r[bd['i']] - r[bd['j']]
        rij -= np.rint(rij/box) * box
        rbd = np.sum(rij*rij)**0.5
        fij = - bd['kr'] * (rbd - bd['r0']) / rbd * rij
        f[bd['i']] += fij
        f[bd['j']] -= fij
    return f
...
def main():
    ...
    for step in range(1, sim['steps']+1):
        ...
        vh = v + f/m * (tstep/2 * 1e-4)
        r = r + vh * tstep
        fbond = forces_bond(r, bonds, box)
        fpair, vir = forces_pair(r, sig, eps, box, rcut, excl)
        f = fbond + fpair
        v = vh + f/m * (tstep/2 * 1e-4)
